...
vec3 addReflectionToPixColor(vec3 lightPos, vec3 dir, hitinfo rayHitPoint, float lightIntensity, int maxBounces)
{
...
vec4 trace(vec3 origin, vec3 dirEyeToTriangle)
{
	...
	if (intersectTriangles(origin, dirEyeToTriangle, eyeHitTriangle))
	{
		...
		for(int j = 0; j < 4; j++)
		{
			...
			vec3 reflection = addReflectionToPixColor(light[j], dirEyeToTriangle, eyeHitTriangle, lightIntensity, 4);

			// In this tutorial, we use Level 4 reflection, you can see that the number 4 is put as a parameter ^^^^^
			// This number controls the maximum amount of bounces that a ray can have, before stopping.
			// Level 4 reflection is not very necessary, there will only be a few pixels that can bounce 4 times
			// and still hit geometry, without going into the void, and they will only be visible for one second. 
